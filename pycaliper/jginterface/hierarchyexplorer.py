...
def ModuleFactory(name, argnames, BaseClass=SpecModule):
...
class HierarchyExplorer:
    ...
    def __init__(self, jgc: JasperConfig, dc: DesignConfig) -> None:
        self.jgc = jgc
        self.dc = dc
    ...
    def get_signals(self, instancename: str) -> dict[str, list[str]]:
        ...
        all_signals = [
            signal for signal in res.split() if signal not in PYCINTERNAL_RESERVED
        ]
        # Only maintain local signals
        if self.topmod == instancename:
            # Top module name is same as instance name
            instance_signals[instancename] = [
                signal for signal in all_signals if "." not in signal
            ]
    ...
    def get_instances(self, instancename: str) -> dict[str, list[str]]:
        ...
        if self.topmod == instancename:
            instance_instances[instancename] = [
                a for a in all_instances if "." not in a and a != self.topmod
            ]
    ...
    def mk_signal(self, instancename: str, signalname: str):
        ...
        logger.debug(
            f"Constructing pycaliper signal: {signalname} from instance {instancename}"
        )
    ...
    def generate_skeleton(self, rootinstance: str = "", maxdepth=-1) -> SpecModule:
        ...
        done_modules: dict[str, SpecModule] = {}

        def gs_helper(_instname: str, depth: int) -> SpecModule:
            ...
            module: SpecModule = moduleclass()

            module._pycinternal__clk = Logic(1, name=self.dc.clk)
            ...
            if depth < maxdepth or maxdepth == -1:
                ...
                for subinstance in inst_instances:
                    ...
                    if _instname == self.topmod:
                        module._pycinternal__submodules[subinstance] = gs_helper(
                            subinstance, depth + 1
                        )
                    else:
                        module._pycinternal__submodules[subinstance] = gs_helper(
                            f"{_instname}.{subinstance}", depth + 1
                        )
            ...
            for signal_basename in inst_signals:
                logger.debug(
                    f"Adding signal: {signal_basename} to instance {_instname} of module {_modname}"
                )
                module._pycinternal__signals[signal_basename] = self.mk_signal(
                    _instname, signal_basename
                )

            done_modules[_modname] = module
            return module

        return gs_helper(rootinstance, 0)
