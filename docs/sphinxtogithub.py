...
class FileHandler(object):
    "Applies a series of replacements to the contents of a file inplace"
    ...
    def process(self):
        with self.opener(self.name, "r", encoding="utf-8") as f:
            text = f.read()

        for replacer in self.replacers:
            text = replacer.process(text)

        with self.opener(self.name, "w", encoding="utf-8") as f:
            f.write(text)
...
def sphinx_extension(app, exception):
    ...
    file_helper = FileSystemHelper(
        lambda f, mode, encoding="utf-8": open(f, mode, encoding=encoding),
        os.path.join,
        shutil.move,
        os.path.exists,
    )
...
def main(args):
    parser = argparse.ArgumentParser(description="Sphinx-to-GitHub utility")
    parser.add_argument("path", help="Path to the HTML directory")
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Provides verbose output"
    )
    parser.add_argument(
        "-e",
        "--encoding",
    ...
    file_helper = FileSystemHelper(
        lambda f, mode, encoding=opts.encoding: open(f, mode, encoding=encoding),
        os.path.join,
        shutil.move,
        os.path.exists,
    )
    ...
    layout = layout_factory.create_layout(opts.path)
    layout.process()
